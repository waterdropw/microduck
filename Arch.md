...
## 七、产品视角的扩展方向（玩法 / 功能 / 形态）

> 依据：`docs/project/roadmap.md`（in flight / missing 清单）、`docs/ideas/autonomous_behavior.md`
>（想法池）、`docs/design/remote-access-design.md`。第一至三节为官方规划，第四、五节为
> 基于现有架构的推演。
...
| 方向 | 状态 | 说明 |
|---|---|---|
| **手机 App**（#107） | 设计完成、未实现 | BLE 配网 + 日常控制；`btd` 的路由子集即为此预留 |
| **SDK** | 规划中 | "一个 API 多种传输"架构的兑现——第三方代码上板 |
| **地图定位 `maploc`**（#127） | 进行中 | ToF 深度 + 里程计 → 建图定位，导航功能的地基 |
| **LAN 外远程** | 设计完成 | 两端鉴权的桥接服务，公网可达的第一步 |
| **自治大脑** | 想法池最重条目 | 16 状态机（Chill/Wander/Zoomies/Startle/Nap/BallPlay/Petted 等）+ 能量/情绪模型 + 新颖度网格探索；所有传感器输入已就位，仅缺状态机本体 |

### 2. 行为玩法扩展（想法池，产品哲学为 "charm per line"——单位代码量的表现力）

**社交层**（全部基于已有的 BLE 广播 + 稳定 id + RSSI，无需新硬件）：

- **认识与问候**：记住见过的鸭子，对老朋友的问候更温暖——友谊以见面次数计量，呈现为记忆
- **孤独/满足**：独处时偶尔呼唤，有同伴时安静——是情绪模型的输入而非独立状态
- **Marco Polo**：一只鸭子藏起，另一只凭 RSSI 距离越近鸣叫越急，为人导航
- **跟随领头鸭**：RSSI 维持间距，ToF 避让前方个体
- **传话**：消息经广播富余字节在鸭子间逐跳传递并发生变异
- **投票**：偏好写入广播负载，多数决定群体下一个行为

**节拍同步动作**（将 ±20 ms 节拍同步能力延伸到视觉——视觉同步容忍 ~50 ms，要求低于音频）：

- 群体点头/摇摆、鸭子火车（conga line）、同步舞蹈

**音乐进阶**：

- 呼应式轮唱（句间空隙天然吸收同步误差，比合唱更易实现）、多声部各司一职（小尺寸扬声器做织体优于做和声）
- 合唱**由命令变为自发事件**："惊喜二重唱是乐趣，点唱机不是"——低概率、以同伴在场为门控

### 3. 感知与智能增强

- **三源融合识别**：摄像头 = 方向，ToF = 距离，BLE = 身份——视觉无法区分两只相同的鸭子，三源融合才能实现"看着对方互动"；duck-detect 已落地，融合为下一步
- **外接 LLM 大脑**：WebSocket 路径已设计完毕，daemon 侧零改动——教育与陪伴场景的想象空间
- **现有功能自主化**：theremin/chorale 由命令式模式折叠为大脑的状态（"Petted" 之于 pet-detect，如同 "Sing" 之于听到的广播）——功能不变，体验从演示变为自发

### 4. 形态拓展（本平台的特殊路径：策略 + 配件，而非更换硬件）

roller 模式已验证该路径：脚底挂被动轮、加载另一份策略文件，双足机器人即变为轮滑机器人。

| 形态 | 所需条件 |
|---|---|
| 更多运动技能包 | 仅需训练新任务（轮滑族已有 6 个）；社区经 `policy add` 可无限扩展 |
| 嘴部精细操作 | beak 硬件已存在，ground_pick 策略已有——精细操作即新策略 |
| 群体编队 | duck-sim 多鸭仿真栈 + BLE 群体协议 |
| 数字孪生 | duck-body 已可将真实 daemon 接入仿真世界 |

### 5. 生态与商业化潜力（推演，非官方规划）

- **社区技能生态**：HF Hub 单策略仓库 + `publish` 工具链，构成去中心化的"技能商店"——第三方策略一条命令上机已在实际发生
- **教育市场**：Onshape CAD → MuJoCo → PPO → ONNX → 真机器人的完整 sim2real 链路，适合高校课程；AGENTS.md 的奖励设计手册本身即教材素材
- **桌面陪伴宠物赛道**：声音人格 + 摸头反应 + 自发行为路线图；800 g / 25 cm / NP-F550 的规格即桌面级

**产品逻辑小结**：硬件平台已收敛（v1 单一硬件配置），所有扩展发生在**策略层（技能）、
行为层（大脑）、生态层（社区/SDK）、远程层（桥接/LLM）**四个软件平面上——这是
"更新器第一个建"的架构红利：机器人本体不变，能力可持续 OTA。
